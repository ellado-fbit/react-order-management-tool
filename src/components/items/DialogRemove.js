...
class DialogRemove extends React.Component {
  ...
  handleButtonRemove = () => {
    ...
    this.props.contracts.forEach(contract => {
      if (contract.item_codes.includes(this.props.register.code)) {
        result = { error: 'Error: El artículo no se puede eliminar porque está siendo usado en contratos.' }
      }
    })
    if (result.error) {
      this.setState({
    ...
    axios.delete(`/items/id/${this.props.register._id}`)
      .then((response) => {
  ...
  render() {
    return (
      <div>
        ...
        >
          <DialogTitle>{"¿Está seguro que desea eleminar el artículo?"}</DialogTitle>
          <DialogContent>
            <Typography variant="subtitle1">
              {`${this.props.register.name} [${this.props.register.code}]`}
            </Typography>
            <br />
