...
class DialogRemove extends React.Component {
  ...
  handleButtonRemove = () => {
    axios.delete(`/contracts/id/${this.props.register._id}`)
      .then((response) => {
        if (response.data.error) {
          ...
          this.setState({
            removedStatus: {
              removed: true,
              error: false,
              message: 'El contrato ha sido eliminado.'
            },
            disableRemoveButton: true
          })
          this.props.onRefresh()
  ...
  render() {
    return (
      <div>
        ...
        >
          <DialogTitle>{"¿Está seguro que desea eleminar el contrato?"}</DialogTitle>
          <DialogContent>
            <Typography variant="subtitle1">
              {`${this.props.register.provider_name} [${this.props.register.store_name}]`}
            </Typography>
            <br />
